...
export default function Sidebar() {
  ...
  // LÓGICA CORRIGIDA para ativar a seção
  const isSectionActive = (sectionItems: NavItem[]) => {
    const hrefs = sectionItems.map(item => item.href);
    const isBaseRouteActive = hrefs.some(href => pathname.startsWith(href));
    const isDetailsRouteActive = pathname.startsWith('/dashboard/solicitacoes/') && hrefs.includes('/dashboard/aprovar-solicitacoes');
    return isBaseRouteActive || isDetailsRouteActive;
  };
  ...
  }, [pathname, navSections, openSections]);
                    ...
                    {openSections.includes(section.name) && (
                      <ul className="mt-1 pl-4 pr-2 space-y-1">
                        {filteredItems.map((item) => {
                          // LÓGICA CORRIGIDA para ativar o sub-item
                          const isSubItemActive = item.href === '/dashboard/aprovar-solicitacoes' 
                               ? pathname.startsWith('/dashboard/solicitacoes/') || pathname === item.href
                               : pathname.startsWith(item.href);
