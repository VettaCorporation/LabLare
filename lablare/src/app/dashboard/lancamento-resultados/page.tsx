...
interface ExameCatalogoData {
  nome_exame: string;
  descricao?: string;
  // Adicione aqui outros campos do catálogo de exames que contenham padrões de referência, se existirem
  // ex: parametros_referencia: string;
}
...
export default function LancamentoResultadosPage() {
  ...
  const [pendingItems, setPendingItems] = useState<ItemSolicitacaoData[]>([]); // Itens pendentes de lançamento
  const [loadingList, setLoadingList] = useState(true);
  const [error, setError] = useState('');

  const [selectedItem, setSelectedItem] = useState<ItemSolicitacaoData | null>(null); // Item selecionado para lançar
  const [resultsInput, setResultsInput] = useState<ParametroResultadoInput[]>([]); // Resultados a serem digitados
  const [observacoesTecnico, setObservacoesTecnico] = useState('');
  const [launching, setLaunching] = useState(false); // Estado de carregamento do lançamento
  ...
  // Função para buscar a lista de amostras pendentes
  const fetchPendingItems = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao carregar lista de amostras pendentes.');
    } finally {
  ...
  const handleSubmitResults = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setResultsInput([]); // Limpa os resultados do formulário
        setObservacoesTecnico(''); // Limpa observações
        fetchPendingItems(); // Recarrega a lista de itens pendentes
      } else {
    ...
    <div className="space-y-6 p-8">
      ...
      {selectedItem && (
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 mt-6">
          <h2 className="text-2xl font-semibold mb-4 text-gray-700">Lançar Resultados para: <span className="text-blue-700">{selectedItem.exame_catalogo.nome_exame}</span></h2>
          ...
          <button
            ...
            disabled={launching || resultsInput.some(r => !r.nome_parametro.trim() || !r.valor_resultado.trim())} // Desabilita se estiver lançando ou campos obrigatórios vazios
