...
const SolicitacoesTable = ({ solicitacoes, onVisualize }: { solicitacoes: SolicitacaoData[], onVisualize: (s: SolicitacaoData) => void }) => {
    return (
                    ...
                    {solicitacoes.map((solicitacao) => (
                        <tr key={solicitacao.id_solicitacao}>
                            ...
                            <td className="px-6 py-4 text-center text-sm font-medium">
                                <button
                                    onClick={() => onVisualize(solicitacao)}
                                    className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded-md text-xs transition duration-200 ease-in-out cursor-pointer"
...
export default function EtiquetaPage() {
  ...
  // --- FUNÇÃO ATUALIZADA PARA REDIRECIONAR ---
  const handleVisualizeExames = useCallback((solicitacao: SolicitacaoData) => {
    router.push(`/dashboard/solicitacoes/${solicitacao.id_solicitacao}/visualizar-exames`);
  }, [router]);
      ...
      <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-800 mt-6">
        ...
        {loadingInitialList || loadingSolicitacoes ? (
            ...
            <SolicitacoesTable
                ...
                onVisualize={handleVisualizeExames}
            />
        )}
      </div>
    </div>
