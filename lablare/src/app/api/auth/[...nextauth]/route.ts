    ...
    providers: [
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                // Incluindo a relação de privilégios na consulta
                    ...
                    include: {
                        perfil: {
                            include: {
                                privilegios: {
                                    select: {
                                        rota: true, // Seleciona apenas o campo 'rota'
                                    },
                                },
                            },
                        },
                ...
                // Mapeia a lista de objetos de privilégios para um array de strings
    ...
    callbacks: {
        async jwt({ token, user }) {
            if (user) {
                ...
                token.privilegios = (user as any).privilegios; // Adiciona os privilégios ao token
            }
            return token;
        },
        async session({ session, token }) {
            if (token) {
                ...
                session.user.nome_perfil = token.nome_perfil as string;
                session.user.isInternalUser = token.isInternalUser as boolean;
                session.user.privilegios = (token as any).privilegios; // Adiciona os privilégios à sessão
            }
            return session;
        },
    },
